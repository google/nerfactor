...
"""A general training and validation pipeline that is simple with minimal
google3 dependecy.
"""
...
# The following import must be here if using XManager hyperparameters.
# It installs a callback into the app initializer, which parses the
# --xm_parameters flag, whose value is a JSON string of all experiment hparams
# pylint: disable=unused-import
import google3.learning.deepmind.xmanager2.client.google as xm # noqa
...
from google3.experimental.users.xiuming.sim.sim.util import \
    logging as logutil, io as ioutil, config as configutil


# Used for reducing the kernel launch time, suggested by xprof
...
def main(_):
    ...
    if not gfile.Exists(config_ini):
    ...
    with strategy.scope():
        ...
        for _ in tqdm(range(step_restored, epochs), desc="Training epochs"):
            ...
            for batch_i, batch in enumerate(datapipe_train):
                # Needed by xprof to trace pure eager-mode training steps
                with tf.profiler.experimental.Trace(
                        'train', step_num=batch_i, _r=1):
                    t0 = time()
...
def maintain_epoch_queue(queue, new_epoch_dir):
    ...
    for epoch_dir in gfile.Glob(join(dirname(new_epoch_dir), '*')):
        if epoch_dir not in queue: # already evicted from queue (FIFO)
            gfile.DeleteRecursively(epoch_dir)
