...
def safe_cumprod(x):
    from google3.vr.kenobi.deep_holo import over_ops

    # over_ops.SaveCumProd assumes axis=0, exclusive=True, reverse=True, so
    # we need to reverse and transpose the array
    r_t_x = tf.reverse(tf.transpose(x, [1, 0]), axis=[0])
    r_t_y = over_ops.SafeCumProd(r_t_x)
    y = tf.reverse(tf.transpose(r_t_y, [1, 0]), axis=[-1])
    return y
