...
logger = logutil.Logger(loggee="models/ns_shape")
...
class Model(BaseModel):
    ...
    def _pred_normal_at(self, pts, eps=1e-6):
        ...
        normal = tf.zeros((tf.shape(pts)[0], 3), dtype=tf.float32)
        for i in tf.range(0, tf.shape(pts)[0], self.mlp_chunk):
            ...
            #
            normal = tf.tensor_scatter_nd_update(
                normal, tf.range(i, end_i)[:, None], chunk)
    ...
    def _pred_lvis_at(self, pts, surf2l):
        ...
        # Chunk by chunk to avoid OOM
        lvis_flat = tf.zeros((tf.shape(surf_flat)[0], 1), dtype=tf.float32)
        for i in tf.range(0, tf.shape(surf_flat)[0], self.mlp_chunk):
            end_i = tf.math.minimum(tf.shape(surf_flat)[0], i + self.mlp_chunk)
            surf_chunk = surf_flat[i:end_i, :]
            surf2l_chunk = surf2l_flat[i:end_i, :]
            # Positional encoding
            surf_embed = self.embedder['xyz'](surf_chunk)
            surf2l_embed = self.embedder['ldir'](surf2l_chunk)
            # Evaluate MLP
            surf_surf2l = tf.concat((surf_embed, surf2l_embed), -1)
            chunk = out_layer(mlp_layers(surf_surf2l))
            #
            lvis_flat = tf.tensor_scatter_nd_update(
                lvis_flat, tf.range(i, end_i)[:, None], chunk)
