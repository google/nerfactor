...
from ..util import logging as logutil, math as mathutil, io as ioutil, \
    img as imgutil
...
class Model(BaseModel):
    ...
    def _compile_into_video(self, batch_dirs, out_webm, fps=12):
        ...
        for batch_dir in tqdm(batch_dirs, desc="Compiling visualized batches"):
            ...
            if not gfile.Exists(json_path) or not gfile.Exists(pred_path):
                logger.warn(
            ...
            nn_path = join(data_root, 'test_phys_nn', id_ + '_nn_*.png')
            nn_path = gfile.Glob(nn_path)
            n_nn = len(nn_path)
            if n_nn == 0:
                frame = pred
            elif n_nn == 1:
