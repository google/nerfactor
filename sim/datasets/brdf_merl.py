...
class Dataset(BaseDataset):
    def __init__(self, config, mode, debug=False):
        ...
        train_pattern = join(self.root, '{mode}*.npz'.format(mode='train'))
        train_paths = sorted(gfile.Glob(train_pattern))
        self.brdf_names = [
        ...
        if mode == 'test':
            path_pattern = join(self.root, '{mode}*.npz'.format(mode=mode))
            paths = sorted(gfile.Glob(path_pattern))
            assert len(paths) == 1, (
    ...
    def _glob(self, seed=0, n_iden=20, n_between=11):
        if self.mode == 'test':
            ...
            path_pattern = join(self.root, '{mode}*.npz'.format(mode=self.mode))
            paths = sorted(gfile.Glob(path_pattern))
